defmodule Durandal.Player.CommandQueries do
  ...
  def _where(query, :subject, subject) do
    from commands in query,
      where: commands.subject in ^List.wrap(subject)
  end
  ...
  def _preload(query, :team) do
    from commands in query,
      left_join: player_teams in assoc(commands, :player_teams),
      preload: [player_teams: player_teams]
  end

  def _preload(query, :user) do
    from commands in query,
      left_join: account_users in assoc(commands, :account_users),
      preload: [account_users: account_users]
  end
end
