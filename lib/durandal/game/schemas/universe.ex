defmodule Durandal.Game.Universe do
  ...
  * `:active?` - field description
  ...
  def changeset(struct, attrs \\ %{}) do
    ...
    tick_seconds = changeset
      |> Ecto.Changeset.get_field(:tick_schedule)
      |> parse_schedule_string()

    next_tick = calculate_next_tick_schedule(Ecto.Changeset.get_field(changeset, :last_tick), tick_seconds)
  ...
  def parse_schedule_string(raw_string) do
    ...
    hours = case Regex.run(~r/(\d+) hours?/, formatted_string) do
      [_, hours] -> String.to_integer(hours)
      nil -> 0
    end

    minutes = case Regex.run(~r/(\d+) minutes?/, formatted_string) do
      [_, minutes] -> String.to_integer(minutes)
      nil -> 0
    end

    seconds = case Regex.run(~r/(\d+) seconds?/, formatted_string) do
