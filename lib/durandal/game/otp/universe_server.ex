defmodule Durandal.Game.UniverseServer do
  ...
  defmodule State do
    @moduledoc false
    defstruct ~w(universe_id universe next_tick topic tick_timer tick_in_progress task_pid)a
  end
  ...
  defp maybe_perform_tick(state) do
    if state.universe.next_tick && DateTime.compare(state.universe.next_tick, DateTime.utc_now()) == :lt do
      state
      |> perform_tick
    else
      state
  ...
  defp perform_tick(state) do
    ...
    server_pid = self()
    {:ok, task_pid} = Task.Supervisor.start_child(task_supervisor, fn ->
  ...
  defp complete_tick(state) do
    ...
    state
    |> struct(%{tick_in_progress: false, task_pid: nil, universe: universe})
  end
