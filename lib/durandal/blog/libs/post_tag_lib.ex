defmodule Durandal.Blog.PostTagLib do
  @moduledoc false
  use DurandalMacros, :library
  alias Durandal.Blog.{PostTag, PostTagQueries}
  ...
  def delete_post_tags(post_id, tag_ids) do
    query = "DELETE FROM blog_post_tags WHERE post_id = $1 AND tag_id = ANY($2);"

    {:ok, _} =
      Ecto.Adapters.SQL.query(Repo, query, [
        Ecto.UUID.dump!(post_id),
        Enum.map(tag_ids, &Ecto.UUID.dump!/1)
      ])
  end
