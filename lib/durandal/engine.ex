defmodule Durandal.Engine do
  ...
  **Instructions:** Unit written orders derived from player commands, much lower level and intended to be easier to transform into actions.
  ...
  @spec tick_universe(Durandal.universe_id()) :: :ok | {:error, [String.t()]}
  def tick_universe(universe_id) do
    t = Task.async(fn ->
      :telemetry.span(
        ...
        fn ->
          ...
          # %{universe_id: universe_id, metadata: "Info1"}
        end
      )
    end)
    Task.await(t)
  ...
  def run_stage(universe_id, stage) do
    :telemetry.span(
      ...
      fn ->
        Map.get(systems_by_stage(), stage, [])
