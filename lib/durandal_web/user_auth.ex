defmodule DurandalWeb.UserAuth do
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_durandal_user_remember_me"
  ...
  def log_in_with_token(conn, %Durandal.Account.UserToken{} = token, params \\ %{}) do
    user_return_to = get_session(conn, :user_return_to)
  ...
  def log_out_user(conn) do
    ...
    cached_user_token &&
      Durandal.invalidate_cache(:user_token_identifier_cache, cached_user_token.identifier_code)

    if live_socket_id = get_session(conn, :live_socket_id) do
      DurandalWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_user:
      ...
      defmodule DurandalWeb.PageLive do
        use DurandalWeb, :live_view

        on_mount {DurandalWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{DurandalWeb.UserAuth, :ensure_authenticated}] do
