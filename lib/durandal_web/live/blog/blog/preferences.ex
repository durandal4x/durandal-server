defmodule DurandalWeb.Blog.BlogLive.Preferences do
  @moduledoc false
  use DurandalWeb, :live_view
  alias Durandal.Blog
  import DurandalWeb.BlogComponents
  alias Durandal.Blog.UserPreferenceLib
  ...
  def mount(_params, _session, socket) do
    socket =
      if is_connected?(socket) do
        tags = Blog.list_tags(order_by: "Name (A-Z)")
  ...
  def handle_event("enable-tag", %{"tag-id" => tag_id}, socket) do
    socket = maybe_make_user_preferences(socket)
  ...
  def handle_event("disable-tag", %{"tag-id" => tag_id}, socket) do
    socket = maybe_make_user_preferences(socket)
  ...
  def handle_event("reset-tag", %{"tag-id" => tag_id}, socket) do
    socket = maybe_make_user_preferences(socket)
  ...
  defp load_preferences(%{assigns: %{current_user: current_user}} = socket)
    ...
    user_preferences = Blog.get_user_preference(current_user.id)
  ...
  defp maybe_make_user_preferences(%{assigns: %{user_preferences: nil}} = socket) do
    ...
    {:ok, new_user_preferences} = Blog.create_user_preference(params)
