  ...
  defmodule Mix.Tasks.Durandal.Task.NewObject do
    ...
    defp schema_file(igniter) do
      ...
      schema_fields =
        ...
        |> Enum.map_join("\n", fn
          [name, "references", table] ->
            "belongs_to(:#{name}, :#{table}, type: Ecto.UUID)"
      ...
      schema_type_fields =
        fields
        |> Enum.map_join(",\n", fn
          [name, "references", table] -> "  #         #{name}_id: #{table}.id()"
        ...
        |> Enum.map_join("\n\n", fn [name, "references", target] ->
          """
            def _preload(query, :#{name}) do
              from objects in query,
                left_join: #{target} in assoc(objects, :#{target}),
    ...
    defp queries_test_file(igniter) do
      ...
      path =
        "test/#{camel_case(@application)}/#{camel_case(context)}/queries/#{camel_case(singular)}_query_test.exs"
      ...
      preload_tests =
        ...
        |> Enum.map_join(", ", fn [_name, "references", target] ->
          ":#{target}"
        end)
      ...
      fixture_fields =
        ...
        |> Enum.map_join(",\n", fn
          [name, "references", _table] ->
            ...
            "#{name}: data[\"#{name}\"] || #{func_name}_fixture().id"
