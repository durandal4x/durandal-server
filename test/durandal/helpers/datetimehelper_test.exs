defmodule Durandal.Helpers.DatetimehelperTest do
  @moduledoc false
  use Durandal.DataCase, async: true
  alias Durandal.Helper.DateTimeHelper

  describe "formatting" do
    test "strftime hms_or_ymd" do
      [
        {~U[2024-02-01 01:02:03Z], "2024-02-01"}
      ]
      |> Enum.each(fn {value, expected} ->
        result = DateTimeHelper.strftime(value, :hms_or_ymd)

        assert result == expected,
          message: "Used #{inspect(value)}, expected: #{expected} but got #{result}"
      end)
    end
  end
