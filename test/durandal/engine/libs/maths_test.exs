defmodule Durandal.Engine.MathsTest do
  ...
  test "invert angle" do
    ...
    for {a1, a2} <- values do
      ...
      result1 = Maths.invert_angle(r1)
      |> round(4)
      assert r2 == result1, message: "Error with first conversion on #{a1},#{a2}, expected #{r2}, got #{result1}"

      result2 = Maths.invert_angle(r2)
      |> round(4)
      assert r1 == result2, message: "Error with second conversion on #{a1},#{a2}, expected #{r1}, got #{result2}"
    end
  end
  ...
  test "angle_distance" do
    ...
    for {a1, a2, expected} <- values do
      ...
      result2 = Maths.angle_distance(Maths.deg2rad(a2), Maths.deg2rad(a1))
      |> round(4)

      assert result2 == expected, message: "Error with #{a1} -> #{a2} reversed, expected #{Maths.deg2rad(expected)} but got #{result2}"
    end
  end
  ...
  test "limit" do
    ...
    for {v, b, expected} <- values2 do
      ...
      assert result == expected, message: "Error with #{v}, #{b}; expected #{expected} but got #{result}"
    end
    ...
    for {v, lb, ub, expected} <- values3 do
      ...
      assert result == expected, message: "Error with #{v}, #{lb}, #{ub}; expected #{expected} but got #{result}"
    end
  end
  ...
  test "limit change" do
    ...
    for {origin, limit_amount, change, expected} <- values do
      ...
      assert result == expected, message: "Error with #{origin}, #{limit_amount}, #{change}; expected #{expected} but got #{result}"
    end
  end
  ...
  test "calculate_angle" do
    ...
    for {p1, p2, _expected = [e1, e2]} <- values3d do
      ...
      msg = "Error with #{inspect p1} -> #{inspect p2}, expected #{inspect {deg(e1), deg(e2)}} but got #{inspect {deg(r1), deg(r2)}}"
