defmodule DurandalWeb.Blog.Blog.IndexLiveTest do
  @moduledoc false
  ...
  describe "Anon Index" do
    ...
    test "viewing the blog", %{conn: conn, post1: post1} do
      ...
      assert_redirect(index_live, ~p"/blog/show/#{post1.id}")

      {:ok, _show_live, html} = live(conn, ~p"/blog/show/#{post1.id}")
      assert html =~ "Post 1 title"
  ...
  describe "Auth Index" do
    setup [:filler_posts, :auth]
    ...
    test "Including preferences", %{conn: conn, user: user, post3: post3, tag1: tag1} do
      ...
      assert_redirect(index_live, ~p"/blog/show/#{post3.id}")

      {:ok, _show_live, html} = live(conn, ~p"/blog/show/#{post3.id}")
      refute html =~ "Post 1 title"
      refute html =~ "Post 2 title"
      assert html =~ "Post 3 title"
