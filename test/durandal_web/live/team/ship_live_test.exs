defmodule DurandalWeb.Live.Team.ShipLiveTest do
  @moduledoc false
  ...
  describe "dashboard" do
    ...
    test "live", %{conn: conn, user: _user, team: team, universe: _universe} do
      ship = Space.get_ship!(nil, where: [team_id: team.id, name: "Team 1 builder"])

      # Lets select then!
      {:ok, view, _html} = live(conn, ~p"/team/ship/#{ship.id}")
